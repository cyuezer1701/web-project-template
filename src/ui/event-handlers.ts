...
import { createItem, deleteItem, subscribeToItems } from '../services/item-service';
...
export function initAppHandlers(renderAppFn: RenderCallback): void {
  ...
  const getStartedBtn = document.getElementById('btn-get-started') as HTMLButtonElement;
  ...
  // "Add Item" button
  const addItemBtn = document.getElementById('btn-add-item') as HTMLButtonElement;
  addItemBtn.onclick = () => {
    (document.getElementById('item-title-input') as HTMLInputElement).value = '';
    (document.getElementById('item-description-input') as HTMLTextAreaElement).value = '';
    showModal('create-item-modal');
    (document.getElementById('item-title-input') as HTMLInputElement).focus();
  };

  // Save item
  const saveItemBtn = document.getElementById('btn-save-item') as HTMLButtonElement;
  saveItemBtn.onclick = async () => {
    const title = (document.getElementById('item-title-input') as HTMLInputElement).value;
    const description = (document.getElementById('item-description-input') as HTMLTextAreaElement)
      .value;
  ...
  // Cancel create item
  const cancelItemBtn = document.getElementById('btn-cancel-item') as HTMLButtonElement;
